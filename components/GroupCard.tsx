...
interface Props {
  group: Group;
  /** Týmy přiřazené do této skupiny */
  teams: Team[];
  /** Zápasy patřící do této skupiny */
  matches: Match[];
...
export function GroupCard({
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
      ...
      {/* Tlačítko rozbalit zápasy */}
      {teams.length > 0 && (
        <>
          <div className="border-t border-gray-800">
            {total === 0 ? (
              <div className="flex items-center justify-between px-5 py-3">
                <span className="text-sm text-gray-500">Žádné zápasy — vygeneruj rozpis.</span>
                <button
                ...
                >
                  {isGenerating ? 'Generuji…' : `Generovat (${teams.length} týmů)`}
                </button>
              </div>
            ) : (
              <button
              ...
              >
                ...
                <svg
                  className={`transition-transform ${matchesOpen ? 'rotate-180' : ''}`}
